...
export default function RecentPostsCard({ platform }) { // Accept platform prop
    const [activeTab, setActiveTab] = useState(platform || 'x'); // Initialize with platform if provided
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            setError('');
            try {
                // Determine platform to fetch: passed prop OR current active tab
                const targetPlatform = platform || activeTab;
                const res = await fetch(`/api/social/recent-posts?platform=${targetPlatform}`);
                if (!res.ok) {
        ...
        fetchPosts();
    }, [activeTab, platform]); // Re-run if tab or prop changes
