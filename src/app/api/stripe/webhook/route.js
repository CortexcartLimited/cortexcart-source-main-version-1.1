  ...
  switch (event.type) {
    case 'checkout.session.completed':
      ...
      if (session.mode === 'subscription') {
        ...
        // 1. Ensure User Exists & Send Welcome Email (CRITICAL FLOW)
        try {
          ...
          if (existingUsers.length === 0) {
            ...
            // User exists
            const userId = existingUsers[0].id; // unused locally but good for log
            await log(`ℹ️ User ${customerEmail} already exists (ID: ${userId}).`);
          }
        } catch (userErr) {
          await log(`❌ FAILED to create user or send email: ${userErr.message}`);
          // We continue even if this fails, to attempt subscription linkage? 
          // Or we treat it as non-fatal for subscription linkage, but fatal for user access.
        }


        // 2. Link Subscription & Update Site Limits (SECONDARY FLOW)
        try {
          ...
          try {
            ...
            await log(`⚠️ Could not retrieve subscription details from Stripe (ID: ${subscriptionId}). internal logic may rely on defaults. Error: ${subErr.message}`);
          }

          if (!priceId) {
            await log(`⚠️ No price ID found. Using default limits.`);
            // You might want to break here or set default priceId
          }

          if (priceId) {
            await log(`🔔 Processing subscription details for ${customerEmail}`);
          ...
          await log(`Error activating subscription logic: ${error.message}`);
        }
      }
      break;
