                        ...
                        <div className="mt-6 space-y-6">
                            {Array.isArray(analysisResult) && analysisResult.length > 0 ? (
                                analysisResult.map((item, index) => (
                                    <div key={index} className="bg-blue-50 border border-blue-100 rounded-lg p-5">
                                        <h5 className="font-bold text-blue-900 mb-2">Original: {item.productName || "Unknown Product"}</h5>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <p className="text-xs font-semibold text-blue-600 uppercase">Suggested Name</p>
                                                <p className="text-gray-800 font-medium">{item.productName || item.suggestedName}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs font-semibold text-blue-600 uppercase">Suggested Description</p>
                                                <p className="text-gray-700 text-sm">{item.productDescription || item.suggestedDescription}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))
                                ...
                                <div className="p-4 bg-yellow-50 text-yellow-800 rounded-md border border-yellow-200">
                                    {analysisResult.message || "No suggestions available."}
                                </div>
                            )}
                        </div>
                    )}
